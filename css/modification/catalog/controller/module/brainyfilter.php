...
class ControllerModuleBrainyFilter extends Controller {
	...
	public function index($moduleSettings) 
    {
        ...
        $conditions = $model->getConditions();
            ...
            $seoSettings = $this->config->get('brainyfilter_seo');
            if ($seoSettings['enabled']) {
                $this->document->addScript('catalog/view/javascript/brainyfilter.seo.js');
                $this->_data['sefParam'] = $seoSettings['param_name'];
                $this->load->model('module/brainyfilter_seo');
                $sefVal = $this->model_module_brainyfilter_seo->getSefUrl($conditions);
                $this->_data['sefValue'] = !empty($sefVal) ? $sefVal : $this->_getRequestParam('bfilter');
            }
...
//            }
	...
	private static function _sortProperties($a, $b)
	{
		return $a['order'] - $b['order'];
            ...
            public function ajaxGetSefUrlParam() {
                $this->load->model('module/brainyfilter_seo');
                $model = new ModelModuleBrainyFilterSeo($this->registry);
                $param = $this->_getRequestParam('bfilter', '');
                $json = array('seoUrl' => null);
                if ($model->isBFParamSef($param)) {
                    $this->load->model('module/brainyfilter');
                    $json['seoUrl'] = $model->getSefUrl($this->model_module_brainyfilter->getConditions());
                }
	...
	public function ajaxfilter()
	{
        ...
        $model->setData($data);
            ...
            $this->load->model('module/brainyfilter_seo');
            $seoModel = new ModelModuleBrainyFilterSeo($this->registry);
            $seoCfg = $this->config->get('brainyfilter_seo');
            if (!isset($model)){
                $model = $this->model_module_brainyfilter;
            }
		...
		if ($isMijoShop) {
            ...
            if (is_string($json)) {
                ...
                $json['seoUrl'] = $seoModel->getSefUrl($model->getConditions());
