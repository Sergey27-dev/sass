...
public function getProductTabs($product_id) {
    ...
    public function getProductElements($product_id) {
        $product_data = array();

        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_complect pc LEFT JOIN " . DB_PREFIX . "product p ON (pc.complect_id = p.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE pc.product_id = '" . (int)$product_id . "' AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'");

        foreach ($query->rows as $result) {
            $product_data[$result['complect_id']] = $this->getProduct($result['complect_id']);
        }
     ...
     */
     public function getTotalProductsOriginal($data = array()) { 
		if (!empty($data['filter_category_id'])) {
			$sql = "SELECT COUNT(*) AS coll FROM  oc_product_to_category WHERE  category_id =".(int)$data['filter_category_id'];
			$query = $this->db->query($sql);
			return $query->row['coll'];
		} else { 
			return 0;
	...
	public function getTotalProductSpecials() {
		...
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "download` d LEFT JOIN `" . DB_PREFIX . "download_description` dd ON (dd.download_id = d.download_id) WHERE d.download_id = '" . (int)$download_id . "'");
		return $query->row;
	}

		public function getDownloads($product_id) {
		$product_downloads_group_data = array();
		$sql = $this->db->query("SELECT * FROM oc_product_to_download as t1 INNER JOIN oc_download as t2 ON t1.download_id = t2.download_id INNER JOIN oc_download_description as t3 ON t2.download_id = t3.download_id WHERE t1.product_id =".$product_id);
