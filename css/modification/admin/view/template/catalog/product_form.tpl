...
<!-- CostPrice end -->
            ...
            <div class="tab-pane" id="tab-links">
              ...
              <div class="form-group">
                <label class="col-sm-2 control-label" for="input-related"><span data-toggle="tooltip" title="<?php echo $help_related; ?>">Элементы системы</span></label>
                <div class="col-sm-10">
                  <input type="text" name="complect" value="" placeholder="Элементы системы" id="input-complect" class="form-control" />
                  <div id="product-complect" class="well well-sm" style="height: 150px; overflow: auto;">
                    <?php foreach ($product_complects as $product_complect) { ?>
                    <div id="product-complect<?php echo $product_complect['product_id']; ?>"><i class="fa fa-minus-circle"></i> <?php echo $product_complect['name']; ?>
                      <input type="hidden" name="product_complect[]" value="<?php echo $product_complect['product_id']; ?>" />
                    </div>
                    <?php } ?>
                  </div>
                </div>
...
// Complect
      $('input[name=\'complect\']').autocomplete({
          'source': function (request, response) {
              $.ajax({
                  url: 'index.php?route=catalog/product/autocomplete&token=<?php echo $token; ?>&filter_name=' + encodeURIComponent(request),
                  dataType: 'json',
          ...
          'select': function (item) {
              $('input[name=\'complect\']').val('');

              $('#product-complect' + item['value']).remove();

              $('#product-complect').append('<div id="product-complect' + item['value'] + '"><i class="fa fa-minus-circle"></i> ' + item['label'] + '<input type="hidden" name="product_complect[]" value="' + item['value'] + '" /></div>');
          }
      });

      $('#product-complect').delegate('.fa-minus-circle', 'click', function () {
          $(this).parent().remove();
      });
//--></script>
  <script type="text/javascript"><!--
